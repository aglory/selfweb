<?php
	...
	function funRenderOperator($o){
		$btn = array();
		if($o['status']==1){
			$btn[] = '<a class="btn status mark" href="'.ActionLink('ajaxmark','message',array('id' => $o['id']),false).'">标记</a>';
		}else if($o['status'] == 2){
			$btn[] = '<a class="btn status unmark" href="'.ActionLink('ajaxunmark','message',array('id' => $o['id']),false).'">取消</a>';
		}
		if($o['status'] != 3){
			$btn[] = '<a class="btn status delete" href="'.ActionLink('ajaxdelete','message',array('id' => $o['id']),false).'">删除</a>';
		}
		return implode('',$btn);
	}
	...
	$sthlist = $pdomysql -> prepare("select * from tbMessageInfo $whereClause $orderBy limit $pageIndex,$pageSize;");
	$sthlist -> execute();
